...
export default function Checkout() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-8">{t('checkout__title')}</h1>
      <form onSubmit={handleSubmit} className="grid md:grid-cols-5 gap-8">
        <div className="md:col-span-3 space-y-6">
          <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 border border-gray-100 dark:border-gray-800 shadow-sm">
            <h2 className="font-semibold text-lg flex items-center gap-2 mb-4 text-gray-900 dark:text-white"><MapPin size={18} /> {t('checkout__shipping')}</h2>
        ...
        <div className="md:col-span-2 space-y-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 border border-gray-100 dark:border-gray-800 shadow-sm">
            <h2 className="font-semibold text-lg flex items-center gap-2 mb-4 text-gray-900 dark:text-white"><CreditCard size={18} /> {t('checkout__summary')}</h2>
            ...
            <div className="border-t dark:border-gray-700 mt-4 pt-4 space-y-2 text-gray-900 dark:text-white"><div className="flex justify-between"><span>{t('cart__subtotal')}</span><span>${total.toFixed(2)}</span></div><div className="flex justify-between text-green-600"><span>{t('cart__shipping')}</span><span>{shippingCost === 0 ? t('checkout__freeShipping') : '$99.00'}</span></div><div className="flex justify-between text-lg font-bold border-t dark:border-gray-700 pt-2"><span>{t('cart__total')}</span><span>${finalTotal.toFixed(2)}</span></div></div>
          </div>
          <button type="submit" disabled={loading} className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-semibold text-lg hover:bg-indigo-700 disabled:opacity-50 transition-all shadow-lg shadow-indigo-200">{loading ? t('checkout__processing') : `${t('checkout__pay')} $${finalTotal.toFixed(2)}`}</button>
        </div>
      </form>
    </div>
  );
