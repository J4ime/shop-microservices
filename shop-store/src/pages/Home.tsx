    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button onClick={() => setSelectedCategory('')} className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${!selectedCategory ? 'bg-indigo-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'}`}>Todos</button>
          {categories.map(cat => (
      ...
      <p className="text-sm text-gray-400 dark:text-gray-500 mb-4">{filtered.length} producto{filtered.length !== 1 ? 's' : ''}</p>
