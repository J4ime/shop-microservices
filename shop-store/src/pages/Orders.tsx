...
export default function Orders() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-8">{t('orders__title')}</h1>
      ...
      orders.length===0 ? <div className="text-center py-20"><Package size={64} className="mx-auto text-gray-300 dark:text-gray-700 mb-4" /><h2 className="text-xl font-bold text-gray-900 dark:text-white mb-2">{t('orders__empty')}</h2><p className="text-gray-500">{t('orders__emptyTagline')}</p></div> :
      <div className="space-y-4">{orders.map(o=>{const I=SI[o.status]||Package;return(<div key={o.id} className="bg-white dark:bg-gray-900 rounded-2xl p-5 border border-gray-100 dark:border-gray-800 shadow-sm"><div className="flex items-center justify-between mb-3"><span className="font-mono text-sm text-gray-500 dark:text-gray-400">{o.orderNumber}</span><span className={`px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1 ${SC[o.status]}`}><I size={14}/>{t(statusKey(o.status))}</span></div><div className="flex items-center gap-3 overflow-x-auto pb-2">{o.items?.map((i:any)=><div key={i.id} className="flex items-center gap-2 bg-gray-50 dark:bg-gray-800 rounded-xl px-3 py-2 flex-shrink-0"><div className="w-10 h-10 bg-gray-200 dark:bg-gray-700 rounded-lg" /><div><p className="text-sm font-medium truncate max-w-[140px] dark:text-white">{i.productName}</p><p className="text-xs text-gray-400">x{i.quantity} · {i.size} · ${i.total.toFixed(2)}</p></div></div>)}</div><div className="flex justify-between items-center mt-3 pt-3 border-t dark:border-gray-700"><span className="text-sm text-gray-400">{new Date(o.createdAt).toLocaleDateString()}</span><span className="text-lg font-bold text-indigo-600 dark:text-indigo-400">${o.total.toFixed(2)}</span></div></div>)})}</div>}
    </div>
  );
}
