...
export default function ProductDetail() {
  ...
  if (!product) return <div className="max-w-7xl mx-auto px-4 py-20 text-center dark:text-white">{t('product__loading')}</div>;
      ...
      <div className="grid md:grid-cols-2 gap-12">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-4">
            ...
            <Star size={18} className="text-gray-300 dark:text-gray-600" /><span className="text-sm text-gray-400 ml-1">4.0 (128 {t('product__reviews')})</span>
          </div>
          ...
          <div className="mt-8"><p className="font-semibold text-gray-900 dark:text-white mb-3">{t('product__size')}: {selectedSize || t('product__selectSize')}</p>
          ...
          <div className="mt-6 flex items-center gap-4"><p className="font-semibold text-gray-900 dark:text-white">{t('product__quantity')}:</p>
          ...
          <button onClick={addToCart} className="mt-8 w-full bg-indigo-600 text-white py-4 rounded-2xl font-semibold text-lg hover:bg-indigo-700 transition-colors flex items-center justify-center gap-2 shadow-lg shadow-indigo-200"><ShoppingCart size={20} /> {t('product__addToCart')} — ${(product.price * quantity).toFixed(2)}</button>
          <div className="mt-6 flex items-center gap-6 text-sm text-gray-500 dark:text-gray-400"><span className="flex items-center gap-1"><Truck size={16} /> {t('product__freeShipping')}</span><span>SKU: {product.sku}</span><span className="capitalize">{product.material}</span></div>
        </div>
      </div>
    </div>
  );
