...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try { await login(email, password); toast.success('Bienvenido'); navigate('/'); }
    catch (err: any) { toast.error('Credenciales inválidas'); }
    setLoading(false);
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-900 px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white">UrbanStyle Backoffice</h1>
        ...
        <form onSubmit={handleSubmit} className="bg-gray-800 rounded-2xl p-6 space-y-4 shadow-xl">
          <div className="relative"><Mail size={18} className="absolute left-3 top-3.5 text-gray-500" />
            <input type="email" required value={email} onChange={e => setEmail(e.target.value)}
              className="w-full pl-10 pr-4 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white text-sm focus:ring-2 focus:ring-indigo-500 outline-none placeholder-gray-500" placeholder="Correo" />
          </div>
          <div className="relative"><Lock size={18} className="absolute left-3 top-3.5 text-gray-500" />
            <input type={showPw ? 'text' : 'password'} required value={password} onChange={e => setPassword(e.target.value)}
              className="w-full pl-10 pr-10 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="Contraseña" />
